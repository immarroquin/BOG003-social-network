  ...
  getPosts().onSnapshot((response) => {
    ...
    response.forEach((doc) => {
      divPosts.innerHTML += `
          ...
          <p>${doc.data().description}</p>
          ${doc.data() && doc.data().likes.includes(uid) ? `
        <img src='img/dislike.png' class='img-like' data-id='${doc.id}'><span>${doc.data().likes.length}</span>`: ` <img src='img/like.png' class='img-like' data-id='${doc.id}'><span>${doc.data().likes.length}</span>` }
          </div>`;
